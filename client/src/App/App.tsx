  ...
  return (
    <div className={style.shift}>
      {props.shift.people.map((person) => (
        <>
          <img width={50} alt="avatar" src={faker.image.avatar()} />
...
function TaskDiv(props: { task: Task; width: number }) {
  return (
    <div key={props.task.id} className={style.shifts} style={{ width: props.width }}>
...
function App() {
  ...
  return (
    <>
      ...
      <div className={style.main}>{tasks ? <TasksTable tasks={tasks} /> : "Loading tasks"}</div>
    </>
  );
}
export default App;
